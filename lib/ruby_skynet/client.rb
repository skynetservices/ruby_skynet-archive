...
module RubySkynet
  class Client
    ...
    def with_connection(&block)
      retries = 0
      begin
        if servers = Registry.servers_for(@service_name, @version, @region)
          ...
          raise "No servers available for service:#{@service_name} with version:#{@version} in region :#{@region}"
        end
    ...
    def new_connection_pool(server)
      ...
      # Cleanup connection pool since that server has gone away
